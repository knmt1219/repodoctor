...
### Fixed
- **npm Execution & Scoped Binary Resolution**: Added package exports map, explicit binary alias `@hominhtuan/repodoctor`, and normalized `./bin/repodoctor.js` path for smooth `npx` / `npm exec` execution across all platforms.
- **Version Synchronization**: Synchronized runtime CLI and report engine version identifiers with package manifest version `0.1.3`.
